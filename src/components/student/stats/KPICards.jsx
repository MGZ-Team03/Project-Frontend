...
function KpiCard({ label, value, changeText, changeTone = 'positive', secondaryText, valueClassName }) {
  const changeClass =
    changeTone === 'positive'
  ...
  return (
    <div className="flex flex-col gap-2 rounded-xl p-6 bg-white dark:bg-[#1a242f] border border-[#dbe0e6] dark:border-gray-800 shadow-sm">
      ...
      <div className="flex items-baseline gap-2">
        <p className={clsx('text-3xl font-black leading-tight text-[#111418] dark:text-white', valueClassName)}>
          {formatValue(value)}
        </p>
        {changeText ? (
...
export default function KPICards({ kpiData, variant, items }) {
  if (variant === 'statics') {
    const normalizedItems = Array.isArray(items) ? items.filter(Boolean) : [];
    const useItems = normalizedItems.length > 0;
    const count = normalizedItems.length;
    const gridClass = !useItems
        ...
        {useItems ? (
          normalizedItems.map((it) => (
            <KpiCard
              ...
              label={it.label}
              value={it.value}
              changeText={it.changeText || null}
              changeTone={it.changeTone || 'positive'}
          ...
          <>
            ...
            <KpiCard
              label="Active Days"
              value={kpiData?.activeDays?.value ?? '0 Days'}
              secondaryText="이번 주"
            />
            <KpiCard
